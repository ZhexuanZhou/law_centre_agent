...
Planner、grader 和 synthesizer 的 JSON 会先做保守语法恢复；仍不能解析或输出因 token 上限被截断时，系统将原响应连同纠错指令重新提交。单次计划最多保留 8 个 query 和 12 个直接相关的精确条文，避免无边界枚举相邻条文。风险计划还会把 query 拆成直接法规、相似案例和补充义务三组；replan 只增量补充，不丢弃首轮意图。Qwen rerank 的 HTTP/JSON 失败也会在上限内重新请求。`case_search` 会扩大重排候选池并优先排列案例；风险任务则使用角色化召回、加权排名融合和最终槽位配额。
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| dev | 16 | 0 | 0.6676 | 0.9375 | 0.8354 | 0.6970 | 0.8854 | 0.7239 | 0.9583 | 0.9375 |
| test | 8 | 0 | 0.5685 | 0.7292 | 0.9375 | 0.7012 | 0.7917 | 0.6113 | 0.7292 | 0.8333 |
| 综合 | 24 | 0 | 0.6345 | 0.8681 | 0.8694 | 0.6984 | 0.8542 | 0.6864 | 0.8819 | 0.9028 |

综合结果按任务拆分：

| 任务 | 推荐观察点 | Recall | RequiredRecall | MRR | nDCG | Coverage | 结论 |
|---|---|---:|---:|---:|---:|---:|---|
| 精确法规 | @1 | 1.0000 | 1.0000 | 1.0000 | 1.0000 | 1.0000 | 严格引用匹配消除了层级编号碰撞，6 条全部位于 Top-1 |
| 案例搜索 | @5 | 0.4167 | 1.0000 | 0.5889 | 0.5020 | 1.0000 | 必需案例和任务覆盖稳定进入结果，但相关案例的覆盖面仍需提高 |
| 跨法域比较 | @10 | 0.8714 | 0.8333 | 1.0000 | 0.8302 | 0.9444 | 整体可用，少数比较目标仍存在缺口 |
| 风险识别 | @10 | 0.4573 | 0.6944 | 0.8889 | 0.5177 | 0.6667 | 角色化检索明显改善召回和覆盖，但仍是当前最弱任务 |

风险优化相对上一轮已推送基线的综合 A/B：

| 指标 @10 | 优化前 | 优化后 | 变化 |
|---|---:|---:|---:|
| Recall | 0.3798 | 0.4573 | +0.0775 |
| RequiredRecall | 0.5833 | 0.6944 | +0.1111 |
| MRR | 0.9167 | 0.8889 | -0.0278 |
| nDCG | 0.4629 | 0.5177 | +0.0548 |
| Coverage | 0.5556 | 0.6667 | +0.1111 |

结论：精确法规检索已经稳定；案例搜索解决了“法条挤占案例槽位”的链路问题，但仍需优化多相关案例召回；跨法域比较已具备较好的覆盖；风险 query 分解、4/2/2 角色配额和加权 rerank 融合带来了明确净收益。风险任务仍然最弱，结构化 coverage 也不等于 gold 证据完整，后续应优先提升相似案例的事实级排序和补充义务精度。
...
结果为 162 项测试通过、24 条 benchmark 全部有效、11,003 个索引 evidence 中不存在 benchmark 悬空引用，Ruff、格式和 diff 检查均通过。
